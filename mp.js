/* Biome World 2P Multiplayer — PeerJS + STUN, short room codes */
(function () {
  ...
  function wire(conn) {
    ...
    conn.on('error', function (err) {
      status('Ошибка связи: ' + (err && err.type ? err.type : 'net'));
    });
  }
  ...
  function host() {
    if (typeof Peer === 'undefined') {
      alert('PeerJS не загрузился. Нужен интернет.');
      return;
    }
    MP.on = true; MP.role = 'host'; disableAI();
    var id = shortId();
    var peer = new Peer(id, PEER_OPTS);
    MP.peer = peer;
    peer.on('open', function (openId) {
      var code = openId || id;
      var el = document.getElementById('mpRoomCode');
      if (el) {
        if (el.tagName === 'INPUT') { el.value = code; el.focus(); el.select(); }
        else el.textContent = code;
      }
      window.__mpRoomId = code;
      var p = document.getElementById('mpPanel'); if (p) p.style.display = 'block';
      status('Комната ' + code + ' — скопируй код');
    });
    peer.on('connection', wire);
    peer.on('error', function (err) {
      var t = err && err.type ? err.type : 'error';
      status('Peer: ' + t + (t === 'unavailable-id' ? ' — создай ещё раз' : ''));
      if (t === 'unavailable-id') {
  ...
  function join(code) {
    ...
    var peer = new Peer(shortId(), PEER_OPTS);
    MP.peer = peer;
    peer.on('open', function () {
      ...
      wire(conn);
      setTimeout(function () {
        if (!MP.ready) status('Не вышло. Хост ждёт; код точный; оба онлайн.');
      }, 12000);
    });
    peer.on('error', function (err) {
      status('Вход: ' + (err && err.type ? err.type : 'ошибка'));
    });
  }
  ...
  function injectUI() {
    ...
    block.innerHTML =
      ...
      '<input id="mpJoinInput" placeholder="Вставь код (bw12ab34)" style="width:100%;margin-top:6px;padding:8px;border-radius:8px;border:1px solid #445;background:#0d1520;color:#e8f0ff;box-sizing:border-box">' +
      '<button class="menuBtn primary" id="mpJoinConfirm" type="button" style="margin-top:8px">Войти</button>' +
      '<div style="margin-top:8px;opacity:.75;font-size:11px">Хост ждёт с открытой игрой. Оба с интернетом. Код: bwxxxxxx</div></div>';
  ...
  console.log('[MP] 2P ready (short codes + STUN)');
})();
